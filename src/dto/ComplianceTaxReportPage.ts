...
/**
 * A paginated page of compliance tax reports (GET /compliance/tax-reports).
 */
export class ComplianceTaxReportPage implements Iterable<ComplianceTaxReport> {
  constructor(
    public readonly rows: ComplianceTaxReport[],
  ...
  static fromResponse(payload: unknown): ComplianceTaxReportPage {
    const obj =
    ...
    const rows = Array.isArray(obj['data'])
      ? (obj['data'] as unknown[])
          ...
          .map(ComplianceTaxReport.fromArray)
      : [];
    ...
    return new ComplianceTaxReportPage(
      rows,
  ...
  [Symbol.iterator](): Iterator<ComplianceTaxReport> {
